...
let gameSpeed, score, gameOver, animReq, isShowcaseView;
...
function init() {
    ...
    if (stack) {
        ...
        if (currentBlock && currentBlock.mesh) scene.remove(currentBlock.mesh);
    }
    stack = [];
    overhangs = [];
    currentBlock = null;
...
function spawnBlock() {
    ...
    addBlock({ x: topBlock.position.x, z: topBlock.position.z, width: topBlock.geometry.parameters.width, depth: topBlock.geometry.parameters.depth, isStatic: false });
}
...
function placeBlock() {
    ...
    const overhangProps = { width: direction === 'x' ? overhang : size, depth: direction === 'z' ? overhang : size };
    ...
    movingMesh.geometry = new THREE.BoxGeometry(direction === 'x' ? newSize : size, 2, direction === 'z' ? newSize : size);
...
function animate() {
    ...
    overhangs.forEach(o => { o.position.y -= 0.15; o.rotation.x += 0.01; o.rotation.z += 0.01; });
    renderer.render(scene, camera);
}

window.addEventListener('click', placeBlock);
window.addEventListener('keydown', (e) => { if (e.code === 'Space') placeBlock(); });
playAgainButton.addEventListener('click', (e) => { e.stopPropagation(); init(); });
